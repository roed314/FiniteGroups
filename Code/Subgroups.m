...
declare attributes SubgroupLatElt:
        ...
        sort_pick, // the subgroup generated by sort_gens.  This can be set without sort_gens if subgroup_count=1
        sort_conj, // an element conjugating subgroup to sort_pick; in the Holomorph if outer_equivalence is true
            ...
            while not HasAttribute(HH, attr) do
                ...
                for cur in current_layer do
                    if (attr eq "normal_closure" and Get(cur, "cc_count") eq Get(cur, "subgroup_count") // normal
                        or attr eq "characteristic_closure" and Get(cur, "subgroup_count") eq 1) then // characteristic
                        HH``attr := cur`i;
                        break;
                    end if;
                    for next in Keys(cur`overs) do
...
{A canonical conjugate of H.  Also sets H`sort_conj, which conjugates H`subgroup to H`sort_pick (note that H`sort_conj will be in the Holomorph if outer_equivalence is true)}
    G := H`Lat`Grp;
    GG := G`MagmaGrp;
    if H`subgroup_count eq 1 then
        H`sort_conj := Identity(H`Lat`outer_equivalence select Get(G, "Holomorph") else GG);
        return H`subgroup;
    end if;
...
{A canonical choice of generators of some conjugate.  H should not have subgroup_count 1 (normal/characteristic)}
    L := H`Lat;
    G0 := L`Grp;
    if L`outer_equivalence then
    ...
    N := L`subs[Get(H, L`outer_equivalence select "characteristic_closure" else "normal_closure")];
    ...
    _, i := Min([x`full_label : x in poss]);
    G := poss[i];
    GG := inj(Get(G, "sort_pick"));
    HH := inj(H`subgroup)^(L`conjugator[[H`i, G`i]] * Get(G, "sort_conj"));
    assert HH subset GG;
    // set gens
    M := #GG;
...
intrinsic sort_key(H::SubgroupLatElt) -> Any
