...
transitive_subgroup | integer | Subgroup label for a subgroup yielding the smallest transitive degree (used for perm_gens if transitive_degree small enough)
...
new_gen           | numeric   | An encoded element that generates `H` together with the smallest numbered maximal subgroup of `H`.  Elements are encoded according to the groups `elt_rep_type` attribute
